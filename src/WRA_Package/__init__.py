...
def load_data(file_path):
    ...
    if file_path.suffix.lower() == '.nc': 
        data = xr.open_dataset(file_path, decode_timedelta=True)  # Open NetCDF file
        df = data.to_dataframe().reset_index()  # Convert to DataFrame and reset index
...
def plot_wind_time_series(df, lat, lon, level=10):
    ...
    """
    Plot wind speed and wind direction time series for a specific location and height.
    ...
    The plots are saved as a PNG file in the "outputs/data_files_you_generate" directory.
    ...
    Parameters:
        ...
        level (int, optional): Height level for the wind data [meters]. Default is 10 m.
    ...
    Raises:
        ValueError: If the input DataFrame does not contain the required columns ('speed', 'direction', 'time').

    Notes:
        - The function saves the generated plot as a PNG file in the "outputs/data_files_you_generate" directory.
        - The file name is dynamically generated based on the latitude, longitude, and height level.
    """
    ...
    axs[0].plot(df['time'],df['speed']) # Plot wind speed against time
    axs[0].set_title(f"Wind Speed Time Series at {level} m [{lat}° N,{lon}° E]") # Set the title for the wind speed plot
    axs[0].set_xlabel('Time') # Label the x-axis
    axs[0].set_ylabel('Wind Speed [m/s]') # Label the y-axis
    axs[0].grid(True) # Add a grid to the plot for better readability

    axs[1].plot(df['time'],df['direction']) # Plot wind direction time series
    axs[1].set_title(f"Wind Direction Time Series at {level} m [{lat}° N,{lon}° E]") # Set the title for the wind direction plot
    axs[1].set_xlabel('Time') # Label the x-axis
    axs[1].set_ylabel('Wind Direction [deg]') # Label the y-axis
    axs[1].grid(True) # Add a grid to the plot for better readability
    ...
    # Generate a dynamic file name based on latitude, longitude, and height level
...
class WindInterpolator:
    ...
    def __init__(self, grid_points, dataframe):
        # Convert list of grid points to a NumPy array for efficient computation
        ...
        for lat, lon in grid_points:
            # Filter dataframe for the current grid point and sort by valid_time
            ...
            # Append wind component values at 10m and 100m heights to respective lists
...
def compute_and_plot_time_series(dataFrame, latitude, longitude, height=10, display_figure=True):
    """
    Compute wind speed and direction time series for a location and height, with optional plotting.

    Parameters:
        dataFrame (pd.DataFrame): Wind data containing 'latitude', 'longitude', 'u10', 'v10', 'u100', 'v100', 'valid_time'.
        ...
        display_figure (bool, optional): Whether to display and save the plot. Default is True.

    Returns:
        df (pd.DataFrame): DataFrame with 'time', 'speed', 'direction', 'u10', 'v10', 'u100', 'v100'.
    ...
    # Check if the given latitude and longitude match any of the defined grid points
    if (latitude, longitude) in grid_points:
        # Filter the DataFrame to only include rows matching the specified latitude and longitude
    ...
    # Create a DataFrame to store the computed wind speed, direction, and components
    ...
    if display_figure == True:
        ...
        return df, fig, axs # Return the DataFrame, figure, and axes
    else: 
...
def calculate_alpha_dynamic(u_10, u_100):
    ...
    """
    # Convert wind speed arrays at 10m and 100m heights to NumPy arrays for computation
    u_10 = np.array(u_10)
    u_100 = np.array(u_100)

    # Suppress warnings for invalid operations (e.g., division by zero) during calculations
    with np.errstate(divide='ignore', invalid='ignore'):
        # Calculate the wind shear exponent (alpha) using the logarithmic wind profile formula
        alpha = np.log(u_100 / u_10) / np.log(100 / 10) # # Logarithmic ratio of wind speeds and heights
...
def extrapolate_wind_speed(u_ref, u_10, u_100, z_ref, z_target):
    ...
    """
    # Calculate the wind shear exponent (alpha) dynamically based on wind speeds at 10m and 100m
...
def plot_wind_rose(direction, speed, num_bins = 6, label_interval = 30): 
    ...
    Notes:
        - The wind rose divides wind direction into 12 sectors (30° each) by default.
    ...
    ax.bar(
            ...
            opening=1.0,  # Set the opening factor for the bars (1.0 = fully open)
